  ...
  return (
    ...
    >
      <span className="shrink-0 w-5 h-5 flex items-center justify-center">
        {numbered ? (
          <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[0.65rem] font-mono shrink-0
            ${active ? 'bg-ink text-amber border border-amber/50 font-bold' : done ? 'bg-safe text-ink font-bold' : 'border border-steel-light'}`}>
            {done && !active ? <Check size={11} strokeWidth={3} /> : numbered}
          </span>
        ) : (
          <Icon size={16} strokeWidth={2} className={active ? 'text-amber' : ''} />
        )}
      </span>
...
export default function SidebarContent({ collapsed = false, onNavigate }) {
  ...
  return (
    <nav aria-label="Main navigation" className="flex-1 overflow-y-auto px-2.5 pb-3">
